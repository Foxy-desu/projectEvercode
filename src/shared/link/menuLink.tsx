...
const DropdownLink = ({ to, text, innerLinks }: IDLinkProps) => {
  ...
  function renderLinks(links: Array<{ to: string; text: string }>) {
    return links.map((link) => (
      <li key={link.to}>
        <NavLink className={cl.innerLink} to={link.to} tabIndex={0}>
          {breakLine(link.text)}
        </NavLink>
      </li>
    ));
  ...
  return (
    <div className={cl.dropdownWrap}>
      ...
      <div className={cl.dropListWrap} data-uk-dropdown={`delay-hide: 200; offset: -0; shift: false; flip:false`}>
        <ul className={dropDownLibClass}>{renderLinks(innerLinks)}</ul>
      </div>
    </div>
  );
};
const IconLink = ({ to, text, location }: ILinkProps) => {
  ...
  return (
    <div style={{ width: '100%' }}>
      <NavLink className={({ isActive }) => `${defaultClass} ${activeClass(isActive)} ${cl.iconLink}`} to={to}>
        {text}
      </NavLink>
    </div>
  );
...
const PlainLink = ({ to, text, location }: ILinkProps) => {
  ...
  return (
    <div style={{ width: '100%' }}>
      <NavLink className={({ isActive }) => `${defaultClass} ${activeClass(isActive)}`} to={to}>
        {text}
      </NavLink>
    </div>
  );
