...
const Header = ({ navMenuLinks, isBurgerOpen, closeBurger, toggleBurger }: IHeader) => {
  ...
  function renderNavLinks(navLinks: INavProps['links']) {
    return navLinks.map((link) => (
      <li key={link.text}>
        <NavLink className={cl.mobileLink} to={link.to} onClick={closeBurger}>
          {link.text}
        </NavLink>
      </li>
    ));
  ...
  return (
    <header className={headerStyle}>
      ...
      <nav className={mobNavStyle}>
        <ul className={cl.navList}>{renderNavLinks(navMenuLinks)}</ul>
      </nav>
    </header>
  );
};
